...
export function detectSkill(text: string): string {
  ...
  if (/\b(aws|cloud)\s*(cost|spend|billing|budget|infra)\b/.test(t) || /\bcost\s*(explorer|anomal|breakd|forecast)\b/.test(t) || /\b(spend|billing)\b/.test(t) || /\bhow\s+m(any|uch)\b.+\b(ec2|ecs|rds|s3|lambda|server|instance|bucket|service|fargate)\b/.test(t) || /\binfra(structure)?\s*(cost|overview|inventory|summary)\b/.test(t)) return "aws-infra";
  return "general";
}
