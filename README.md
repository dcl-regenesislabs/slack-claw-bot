...
- Scheduled runs cannot manage schedules: the schedules-file path is delivered per-run through a trusted prompt header that only interactive Slack runs receive, and scheduled runs' tools block writes to the schedules directory — so injected content in a polled source can't rewrite the schedule set.
