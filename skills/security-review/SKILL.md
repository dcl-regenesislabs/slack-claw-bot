...
description: Run a security review pass on code changes — check for secrets, injection vulnerabilities, auth/authz gaps, sensitive data exposure, and dependency risks. Runs automatically as part of every pr-review; also use when asked to review security directly.
---
...
- No secrets committed to git history (check the full diff, not just the final state)
...
- Prompt injection: untrusted text (user messages, fetched content) not interpolated into LLM prompts as instructions
...
- No secrets in issue bodies, PR descriptions, comments, or Slack replies
...
# Newly added packages in the diff
gh pr diff {number} -R {owner}/{repo} -- package.json package-lock.json
```

Run `npm audit` (or the ecosystem's equivalent) and flag high/critical findings. Eyeball any newly added package: does it look legitimate, maintained, and proportionate to what it's used for?
...
- **High** — likely exploitable with effort (e.g. missing input validation on a sensitive path)
- **Medium** — exploitable under specific conditions
- **Low** — defense-in-depth improvement (e.g. missing rate limit on a low-value endpoint)

Reference the exact file:line for each finding and suggest a concrete fix.
