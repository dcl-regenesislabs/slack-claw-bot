...
const IMAGE_MAGIC: [string, Buffer][] = [
...
async function downloadSlackFile(
  ...
  if (contentType.includes("text/html")) {
    console.error(
      `[slack] download ${filename}: expected ${mimeType} but got Content-Type "${contentType}" (likely auth redirect or error page)`,
    );
    throw new Error(`Unexpected Content-Type "${contentType}" for ${filename}`);
  }

  const buffer = Buffer.from(await response.arrayBuffer());

  const isImageMime = IMAGE_MIMES.has(mimeType);
  if (isImageMime && !looksLikeImage(buffer)) {
    const hex = buffer.subarray(0, 16).toString("hex");
    console.error(
      `[slack] download ${filename}: data does not match any known image magic bytes (first 16 bytes: ${hex}, Content-Type: "${contentType}", expected: ${mimeType})`,
    );
    throw new Error(`Downloaded data for ${filename} is not a valid image`);
  }
